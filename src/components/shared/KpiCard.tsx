...
export default function KpiCard({
  ...
  return (
    <div
      className={`bg-white rounded-xl px-5 py-4 border border-gray-100 shadow-card hover:shadow-card-hover transition-all duration-200 border-t-[3px] ${topBorder}`}
    >
      {/* Label row */}
      <p className="text-[10.5px] font-semibold tracking-[0.09em] uppercase text-gray-400 flex items-center gap-1.5">
        {label}
        {hint && (
      ...
      {/* Primary value */}
      <p className="text-[30px] font-bold text-gray-900 mt-2 leading-none tracking-tight tabular-nums">
        {value}
      </p>

      {/* Sub-value */}
      {subValue && (
        <p className="text-[11.5px] text-gray-500 mt-1.5 leading-snug">{subValue}</p>
      )}
