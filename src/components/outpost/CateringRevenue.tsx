  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-sm border-l-4 border-outpost-black">
      ...
      <button
        ...
        className="mt-3 flex items-center gap-1 text-xs font-medium text-gray-500 hover:text-gray-800 transition-colors"
      >
        <ChevronDown
          size={13}
          className={`transition-transform duration-200 ${showItems ? 'rotate-0' : '-rotate-90'}`}
        />
        {showItems ? 'Hide items' : `Show ${items.length} items`}
      </button>
      {showItems && (
        <>
